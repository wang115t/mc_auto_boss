...
def find_text(targets: str | list[str], img: np.ndarray = None, return_position: bool = False) -> OcrResult | None or tuple[int, int] | None:
...
def check_boss(bossName, is_wait: bool = False):
    ...
    if contains_any_combinations(bossName, keywords_turtle, min_chars=2):
        ...
        info.echoSearchModel = "yolo.onnx"
        wait_time = 7
    elif contains_any_combinations(bossName, keywords_dreamless, min_chars=3) or contains_any_combinations(bossName, keywords_dreamless_new, min_chars=5):
        info.bossTrueName = "无冠者之像"
        info.echoSearchModel = "heart.onnx"
        wait_time = 3
        info.inDungeon = True
    elif contains_any_combinations(bossName, keywords_jue, min_chars=1) or contains_any_combinations(bossName, keywords_jue_new, min_chars=5):
